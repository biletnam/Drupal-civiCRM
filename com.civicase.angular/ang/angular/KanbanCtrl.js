...
// AngularKanbanCtrl controller. 
(function(angular, $, _) {

    // Angular, Drag and drop modules
    angular.module('angular', ['drag-and-drop']).config(function($routeProvider) {
    $routeProvider.when('/Kanban', {
    controller: 'AngularKanbanCtrl',
    templateUrl: '~/angular/KanbanCtrl.html', // Template file
    ...
    resolve: {
        init:function()
        {
            // add loader div before template data load
            angular.element(document.getElementById('crm-main-content-wrapper')).append("<div id='loaderdiv'>Loading... Please wait.</div>");
        },
        // Status 1 case data
        myCases_status1: function(crmApi) {
        return CRM.api3('Case', 'get', {
        sequential:1,
                status_id:1
        ...
        // Status 2 case data
        myCases_status2: function(crmApi) {
        return CRM.api3('Case', 'get', {
        sequential:1,
                status_id:2
        ...
        // Status 3 case data
        myCases_status3: function(crmApi) {
        return CRM.api3('Case', 'get', {
        sequential:1,
                status_id:3
        ...
        // Contact detail
        myContact_detail: function(crmApi) {
        return CRM.api3('Contact', 'get', {
        sequential:1,
                options:{limit: - 1},
        ...
        // Activity log
        myActivity_log: function(crmApi) {
        return CRM.api3('ActivityContact', 'get', {
        sequential:1,
                options:{limit:0},
        });
        }
    ...
    angular.module('angular').controller('AngularKanbanCtrl', function($scope, crmApi, crmStatus, crmUiHelp, myContact_detail, myActivity_log, myCases_status1, myCases_status2, myCases_status3, $http) {
    ...
    // remove loader div
    angular.element( document.querySelector( '#loaderdiv' ) ).remove();
    $scope.myContact_detail = myContact_detail.values;
    $scope.myActivity_log = myActivity_log.values;
    $scope.list1 = myCases_status1.values;
    $scope.list2 = myCases_status2.values;
    $scope.list3 = myCases_status3.values;
      
    // Ondrop custom function 
    $scope.update_item = function(event, ui) {
    var statausid = event.target.attributes['data-val'].value; // Get statusid for update
    var curid = ui.draggable.scope().item.id; // get case id for update
